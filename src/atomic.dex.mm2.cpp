...
namespace atomic_dex
{
	...
	void mm2::update() noexcept
	{
		...
		std::chrono::seconds s = std::chrono::duration_cast<std::chrono::seconds>(now - orderbook_clock_);
		std::chrono::seconds s_info = std::chrono::duration_cast<std::chrono::seconds>(now - info_clock_);
		if (s >= 5s)
		{
			orderbook_flow_.clear();
			orderbook_flow_.emplace([this]()
			{
				this->fetch_current_orderbook_thread();
			});
			executor_.run(orderbook_flow_);
			orderbook_clock_ = std::chrono::high_resolution_clock::now();
		}
		...
		{
			info_flow_.clear();
			info_flow_.emplace([this]()
			{
				this->fetch_infos_thread();
			});
			executor_.run(info_flow_);
			info_clock_ = std::chrono::high_resolution_clock::now();
		}
	}
	...
	bool mm2::enable_coin(const std::string& ticker) noexcept
	{
		...
		taskflow.emplace([this, ticker]()
		{ process_balance(ticker); }, [this, ticker]()
		{ process_tx(ticker); });
	...
	bool mm2::enable_default_coins() noexcept
	{
		...
		{
			taskflow.emplace([this, current_coin]() { this->enable_coin(current_coin.ticker); });
		}
		executor.run(taskflow).wait();
	...
	void mm2::fetch_infos_thread()
	{
		...
		for (auto&& current_coin : coins)
		{
			taskflow.emplace([this, current_coin]()
