...
namespace atomic_dex
{
        ...
        QAbstractListModel(parent), m_wallet_manager(wallet_manager_)
    {
        ...
        SPDLOG_DEBUG("contact model created");
    }

    contact_model::~contact_model() noexcept
    {
        SPDLOG_DEBUG("{} l{} f[{}]", __FUNCTION__, __LINE__, fs::path(__FILE__).filename().string());
        SPDLOG_DEBUG("contact model destroyed");
    }
    ...
    atomic_dex::contact_model::set_name(const QString& name) noexcept
    {
        if (name != m_name)
        {
            SPDLOG_DEBUG("name {} changed to {}", m_name.toStdString(), name.toStdString());
    ...
    atomic_dex::contact_model::setData(const QModelIndex& index, const QVariant& value, int role)
    {
        ...
        case TypeRole:
            ...
            {
                SPDLOG_DEBUG("changing contact {} ticker {} to {}", this->m_name.toStdString(), item.type.toStdString(), value.toString().toStdString());
        ...
        case AddressRole:
            if (value.toString() != item.address)
            {
                ...
                SPDLOG_DEBUG("changing contact {} ticker {} to address {}", this->m_name.toStdString(), item.type.toStdString(), item.address.toStdString());
    ...
    {
        SPDLOG_DEBUG("(contact_model::insertRows) inserting {} elements at position {}", rows, position);
    ...
    {
        SPDLOG_DEBUG("(contact_model::removeRows) removing {} elements at position {}", rows, position);
