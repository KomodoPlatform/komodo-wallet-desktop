...
namespace atomic_dex
{
        ...
        QAbstractListModel(parent), m_wallet_manager(wallet_manager_), m_addressbook_proxy(new addressbook_proxy_model(this))
    {
        ...
        SPDLOG_DEBUG("addressbook model created");
    ...
    {
        SPDLOG_DEBUG("{} l{} f[{}]", __FUNCTION__, __LINE__, fs::path(__FILE__).filename().string());
        SPDLOG_DEBUG("addressbook model destroyed");
    }
    ...
    {
        SPDLOG_DEBUG("(addressbook_model::insertRows) inserting {} elements at position {}", rows, position);
    ...
    {
        SPDLOG_DEBUG("(addressbook_model::removeRows) removing {} elements at position {}", rows, position);
    ...
    atomic_dex::addressbook_model::initializeFromCfg()
    {
        ...
        auto functor = [this](const atomic_dex::contact& cur_contact) {
            int position = 0;
            int rows     = 1;
            SPDLOG_DEBUG("(addressbook_model::initializeFromCfg) inserting {} elements at position {}", rows, position);
