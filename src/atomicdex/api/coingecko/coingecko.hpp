...
namespace atomic_dex::coingecko::api
{
    ...
    void from_json(const nlohmann::json& j, market_infos_answer& answer);
    ...
    ENTT_API std::vector<std::string> from_enabled_coins(const t_coins_registry& coins);

    pplx::task<web::http::http_response> async_market_infos(market_infos_request&& request);
