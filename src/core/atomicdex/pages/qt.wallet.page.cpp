    ...
    wallet_page::set_validate_address_data(QVariant rpc_data)
    {
        ...
        if (json_result.contains("reason"))
        {
            ...
            if (!reason.isEmpty())
            {
                ...
                {
                    reason =
                        tr("Invalid checksum for %1.\nClick the button to convert to mixed case address.").arg(json_result["ticker"].toString());
                ...
                {
                    reason =
                        tr("Legacy address used for %1.\nClick the button to convert to a Cashaddress.").arg(json_result["ticker"].toString());
