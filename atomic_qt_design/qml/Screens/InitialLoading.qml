...
SetupPage {
    ...
    property Timer check_loading_complete: Timer {
        ...
        repeat: true
        onTriggered: {
            if(API.get().initial_loading_status === "complete" && API.get().portfolio_pg.portfolio_mdl.length >= 2) {
                running = false
                onLoaded()
            }
        }
        ...
        RowLayout {
            ...
            DefaultText {
                text_value: API.get().settings_pg.empty_string + ((API.get().initial_loading_status === "initializing_mm2" ? qsTr("Initializing MM2") :
                       ...
                       API.get().initial_loading_status === "complete" && API.get().portfolio_pg.portfolio_mdl.length >= 2 ? qsTr("Complete") : qsTr("Getting ready")) + "...")
            }
        }
    }
}
