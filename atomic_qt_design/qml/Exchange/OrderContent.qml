...
Item {
    ...
    readonly property bool is_placed_order: details.order_id !== ''

    // Base Icon
    DefaultImage {
        id: base_icon
        source: General.coinIcon(details.base_coin)
    ...
    DefaultImage {
        id: rel_icon
        source: General.coinIcon(details.rel_coin)
        fillMode: Image.PreserveAspectFit
    ...
    DefaultText {
        id: base_amount
        text_value: API.get().empty_string + ("~ " + General.formatCrypto("", details.base_amount, details.base_coin))
    ...
    DefaultText {
        id: rel_amount
        text_value: API.get().empty_string + ("~ " + General.formatCrypto("", details.rel_amount, details.rel_coin))
    ...
    DefaultText {
        ...
        text_value: API.get().empty_string + (qsTr("ID") + ": " + details.order_id)
...
//    DefaultText {
//        visible: !in_modal && (details.events !== undefined || details.is_maker === false)
...
//    }
    ...
    DefaultText {
        id: date
        visible: !in_modal && details.date !== ''
        text_value: API.get().empty_string + (details.date)
        color: Style.colorTheme2
    ...
    // Cancel button
    DangerButton {
        visible: !in_modal && details.cancellable !== undefined && details.cancellable
        anchors.right: parent.right
...
//    PrimaryButton {
//        visible: !in_modal && details.is_recoverable !== undefined && details.is_recoverable
//        anchors.right: parent.right
//        anchors.bottom: date.bottom
