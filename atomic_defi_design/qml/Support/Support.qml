...
If you go offline, so will your orders, and any that are in progress will fail, leading to potential loss of trade / transaction fees, and a wait for the swap to timeout and issue a refund. It may also negatively affect your wallet's eputation score for future trade matching.
...
For this reason, we recommend cancelling orders before closing the %1, or reviewing and revising your prices when restarting %1.").arg(API.app_name)
            }
