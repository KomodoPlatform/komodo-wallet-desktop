...
proc generate_solution*(build_type: string, osx_sdk_path: string, compiler_path: string) =
    ...
    if not os.existsDir(${PROJECT_ROOT_DIR}.joinPath(full_name)):
    ...
    os.setCurrentDir(${PROJECT_ROOT_DIR}.joinPath(full_name))
    assert(os.existsEnv("QT_INSTALL_CMAKE_PATH"))
    var cmd_line = "cmake -GNinja -DCMAKE_BUILD_TYPE=" & build_type & " " & ${PROJECT_ROOT_DIR}
    when defined(osx):
