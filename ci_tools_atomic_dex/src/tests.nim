...
proc run_tests*(build_type: string, osx_sdk_path: string, compiler_path: string) =
     ...
     when defined(osx):
        ...
        os.setCurrentDir(os.getCurrentDir().joinPath("bin").joinPath("atomicDeFi_tests.app").joinPath("Contents").joinPath("MacOS"))
        echo "Running AtomicDex Pro Unit tests"
        discard osproc.execCmd("./atomicDeFi_tests --reporters=xml --out=atomic-dex-tests-result.xml -s")
     ...
     when defined(windows):
        ...
        echo "Running AtomicDex Pro Unit tests"
        discard osproc.execCmd(".\\atomicDeFi_tests.exe --reporters=xml --out=atomic-dex-tests-result.xml -s")
        echo "Successfully Generated atomic-dex-tests-result.xml"
